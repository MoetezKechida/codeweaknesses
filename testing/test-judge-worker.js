/**
 * Judge Worker Test - Real Docker-based Code Execution
 * Tests the complete flow with actual code execution and test cases
 */
...
async function runJudgeWorkerTest() {
  ...
  try {
    ...
    console.log('⚙️  Step 5: Submitting correct JavaScript code...');
    const correctCode = `
console.log(1);
console.log(2);
console.log(3);
`.trim();
    ...
    console.log(`   ⏱️  Total execution time: ${submission.executionTime}ms`);
    ...
    if (submission.status === 'accepted' && submission.score === 100) {
      ...
      console.log('⚠️  Submission marked as WRONG');
      console.log(`   Some test cases failed. Score: ${submission.score}%\n`);
    } else {
      console.log(`⚠️  Submission in ${submission.status} status\n`);
    }
